...
bool DotsHandler::handle(ESP8266WebServer &server, HTTPMethod method,
                         String uri) {
  if (!canHandle(method, uri)) {
    return false;
  }

  debug_printf("Handle '%s' ? '%s' (%s) in DotsHandler...", uri.c_str(),
               argsToString(server).c_str(),
               method == HTTP_GET ? "GET" : "POST");

  if (method == HTTP_POST && server.hasArg("dots")) {
    String dots = server.arg("dots");
    unsigned char *dots_bytes =
        (unsigned char *)malloc(dots.length() * sizeof(unsigned char));
    ...
    for (i = 0; i < dots_bytes_length; ++i) {
      Serial.printf("%u|", dots_bytes[i]);
    }
    Serial.printf("\n");

    if (dots_bytes_length !=
        (flippie->config->num_rows * flippie->config->num_modules * 4)) {
      Serial.printf("Size mismatch: 'dots' is %u instead of %u bytes (%u rows "
                    "X %u modules X 32-bit-int(4 bytes)) long!\n",
                    dots_bytes_length, (flippie->config->num_rows *
                                        flippie->config->num_modules * 4),
                    flippie->config->num_rows, flippie->config->num_modules,
                    flippie->config->num_rows, flippie->config->num_modules);
      return false;
    }

    unsigned int **int_dots = (unsigned int **)malloc(
        flippie->config->num_rows * sizeof(unsigned int *));
    unsigned int ptr = 0;
    for (i = 0; i < flippie->config->num_rows; i++) {
      int_dots[i] = (unsigned int *)malloc(flippie->config->num_modules *
                                           sizeof(unsigned int));
      for (j = 0; j < flippie->config->num_modules; ++j) {
        int_dots[i][j] = (dots_bytes[ptr]) | (dots_bytes[ptr + 1] << 8) |
                         (dots_bytes[ptr + 2] << 16) |
                         (dots_bytes[ptr + 3] << 24);
        ptr += 4;
      }
    ...
    unsigned int dots_bytes_length =
        flippie->config->num_rows * flippie->config->num_modules * 4;
    ...
    b64.encode(dots_bytes, dots_bytes_length, dots_string + 1,
               &dots_string_length);
    dots_string[0] = '"';
